...
本设计完整代码（训练、评估、可视化脚本及ROS2部署包）已开源于 GitHub：
\url{https://github.com/mischievousx/H1_Walking_Genesis}。
...
\begin{enumerate}
    \item \textbf{平滑性与跟踪性能的权衡。}
          增大 $|c_\text{dof\_acc}|$ 可显著降低关节加速度（表~\ref{tab:eval_breakdown}中
          \texttt{dof\_acc} 由 $51.26\times10^{-3}$ 降至 $23.03\times10^{-3}$，降幅约
          $55\%$），同时 \texttt{dof\_vel\_jitter} 与 \texttt{dof\_jerk} 分别下降约
          $33\%$ 和 $34\%$（表~\ref{tab:joint_smoothness}），从而减小传动系统冲击载荷；
          但过强的惩罚（40×）会在高速指令下牺牲一定的跟踪精度，且平滑性指标不降反升。
